<template>
	<header class="GlobalHeader z-10 py-6 lg:py-14 mb-8 sticky">
		...
		>
			...
			<div class="">
				<GlobalHeaderMenu
					class="flex lg:justify-center relative z-10 max-h-[1.5rem]"
				/>
			</div>
			...
			<div class="col-start-2 lg:col-start-3 row-start-1">
				<ul
					class="flex items-center justify-end gap-4 lg:gap-6 translate-y-1 lg:translate-y-0.5"
				>
					<li v-for="logo of data?.body" :key="logo.src">
						<NuxtLink
							:to="logo.url"
...
<script lang="ts" setup>
	...
	const { data } = await useAsyncData('GlobalHeader', () =>
		queryContent<{ body: { title: string; src: string; url: string }[] }>(
